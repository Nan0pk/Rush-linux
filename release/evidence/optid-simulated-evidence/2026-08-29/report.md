...
- 863 receipts recorded; 827 actions became active on the simulated machine; 822 of those restored to their previous value.
...
**Domains that actuated in a configuration that had switched them off.** This can only happen through the policy-reload fallback described in the findings below.

- `full_observe`: cpu_dma_latency, device_resume_latency, platform_profile, vm_sysctl
- `off_all_domains`: cpu_dma_latency, device_resume_latency, platform_profile, vm_sysctl
...
### `policy_reload_fallback_escalates_domain_modes` — high

A policy file that becomes unparseable while optid is running is replaced by `Policy::curated_baseline()`, whose per-domain default is `actuate`. The run loop reloads the policy on every cycle but computes `apply_armed` and the load state only at startup, so an operator's explicit `mode = "off"` (or `observe`) configuration silently becomes actuation mid-run.

- off_all_domains / config_reload_failure_and_recovery: cpu_dma_latency:cpu moved unconstrained -> 1000 even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: device_resume_latency:0000:00:1f.6 moved 0 -> 10000 even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: device_resume_latency:0000:01:00.0 moved 0 -> 10000 even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: device_resume_latency:0000:02:00.0 moved 0 -> 10000 even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: platform_profile:acpi moved balanced -> balanced even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: vm_sysctl:dirty_background_bytes moved 0 -> 67108864 even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: vm_sysctl:dirty_bytes moved 0 -> 134217728 even though every domain is configured off
- off_all_domains / config_reload_failure_and_recovery: vm_sysctl:swappiness moved 60 -> 100 even though every domain is configured off
- full_observe / config_reload_failure_and_recovery: cpu_dma_latency:cpu moved unconstrained -> 1000 even though every domain is configured observe
- full_observe / config_reload_failure_and_recovery: device_resume_latency:0000:00:1f.6 moved 0 -> 10000 even though every domain is configured observe
