...
- `rushbench pair-plan --pairs <n> --seed <u64> [--out <file>]`
  Produces a deterministic balanced AB/BA matched-pair order for controlled comparisons. The JSON output records the seed and exact order so the ordering of a physical campaign can be reproduced exactly; generating the plan performs no benchmark or hardware mutation.
...
unrelated process. Any descendants that survived such an early leader exit may
contaminate later phases, so a run with that condition is not trustworthy
benchmark evidence until run-level tainting or identity-checked descendant
cleanup is added. This is a software ownership boundary, not proof that every
