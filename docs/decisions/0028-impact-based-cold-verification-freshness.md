# ADR 0028 — Impact-based freshness for completed-package verification

- Status: accepted
- Date: 2026-09-13
- Decision authority: owner
- Trigger: runtime power-management PR #471 exposed that the existing proof-path freshness rule requires full cold re-verification of every completed package whenever any declared proof file changes, even when the package's proven behavior is unaffected.
...
The replacement keeps the safety property at the level that matters: **a completion claim may survive only when an independent reviewer establishes that the later change did not invalidate the proof**. Full cold verification remains mandatory when that cannot be established.
...
The validator may still surface touched proof paths as review context, but such a touch alone is not a package-state failure.
