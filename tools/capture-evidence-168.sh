...
# Requires: root + KVM, and: mkosi, qemu-system-x86_64, edk2/OVMF, mtools, sgdisk,
#           systemd-repart, losetup, cargo/rust.
#   Arch:   sudo pacman -S --needed mkosi qemu-base edk2-ovmf mtools gptfdisk rust base-devel
#   Fedora: sudo dnf install -y mkosi qemu-system-x86 edk2-ovmf mtools gdisk cargo
set -euo pipefail
cd ~/Rush-linux

# 0. Fresh main + work branch (strict policy: branch must be up to date)
git checkout main && git pull --ff-only origin main
git checkout -B evidence/dragnet-168-build-host
...
tools/validate-uefi-boot.sh build/rush-linux.raw 2>&1 | tee build/uefi-boot-capture.log
BOOT=build/uefi-boot.log
grep -E "multi-user\.target"            "$BOOT" || { echo "ABORT: no multi-user marker"; exit 1; }
...
tools/test-rollback.sh build/rush-linux.raw 2>&1 | tee build/rollback-capture.log
...
sudo bash tools/test-install.sh build/rush-linux.raw 2>&1 | tee build/install-capture.log
...
# 5. Flip the 10 flags + add transcript= + mark milestones complete (you = the human)
python3 - <<'PY'
...
# 6. Verify gate GREEN (verified=true => committed transcript) + Dragnet GREEN
python3 tools/validate-evidence.py
python3 tools/dragnet.py --observe        # expect VERDICT: GREEN

# 7. Commit + push (PR is gated by the required "Evidence integrity (Dragnet)" check)
git add release/evidence release/milestones.toml
git commit -m "evidence(#168): build-host acceptance transcripts; close v0.3/v0.4/v0.5"
git push -u origin evidence/dragnet-168-build-host
echo "NEXT: open a PR for evidence/dragnet-168-build-host -> main; merge once green."
echo "Then the 0.6.0-beta.1 bump is unlocked (VERSION + Cargo.toml + ROADMAP.md + milestones current_version, then validate-versions.py)."
