...
def _execute_validation_step(
    ...
    if not bundle_path.is_absolute():
        bundle_path = run_dir / bundle_path if not repo_root else repo_root / bundle_path

    # STEP 4A hardening: verify the hwtest evidence bundle exists before
    # running validation. In real (non-fake) runs, the evidence bundle is
    # generated by _generate_evidence_bundle() AFTER the plan steps complete.
    # But validation steps are part of the plan — so the bundle may not exist
    # yet when the validation step runs. We must check and fail clearly if
    ...
    missing = [f for f in required_files if not (bundle_path / f).exists()]
    if missing:
        return StepResult(
            seq=seq,
            status="failed",
            error=f"validation cannot run: missing bundle files: {missing}. "
                  f"Evidence bundle must be generated before validation. "
