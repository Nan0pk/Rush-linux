...
case "$SECTION" in
    all|integrity|docs|optid|evidence|policy|workflow|shell|powershell|python|rust) ;;
    *)
        echo "Unknown section: $SECTION" >&2
        echo "Valid sections: integrity docs optid evidence policy workflow shell powershell python rust" >&2
        exit 2
        ;;
esac
...
record_failure() {
    local risk="$1" command="$2" status="${3:-1}"
    FAILURES=$((FAILURES + 1))
    FAILED_RISKS+=("$risk")
    ...
    if [[ "${GITHUB_ACTIONS:-false}" == "true" ]]; then
        printf '::error title=Rush CI check failed::%s\n' \
            "$(github_escape "$risk | exit $status | reproduce: $command")"
    fi
}
...
echo "Rush checks: section=$SECTION mode=$MODE base=$BASE"

if selected integrity; then
...
if selected policy; then
    if need pwsh "repository policy"; then
        ...
        record_failure "R4/R8 — repository policy checks could not run" \
            "install pwsh and rerun bash tools/checks.sh --section policy" 127
    fi
fi

if [[ "$MODE" == "quick" && "$SECTION" == "all" ]]; then
    if (( FAILURES > 0 )); then
        write_failure_summary
        exit 1
    fi
...
if selected workflow && matches '^\.github/workflows/.*\.ya?ml$'; then
    if need actionlint "GitHub Actions workflow"; then
        ...
        record_failure "R5 — GitHub Actions workflow checks could not run" \
            "install actionlint and rerun bash tools/checks.sh --section workflow" 127
    fi
fi
    ...
    if (( ${#SHELL_FILES[@]} > 0 )); then
        ...
        if need shellcheck "shell static analysis"; then
            ...
            record_failure "R5 — shell static analysis could not run" \
                "install shellcheck and rerun bash tools/checks.sh --section shell" 127
        fi
    fi
fi

if selected powershell && matches '\.ps1$'; then
    if need pwsh "PowerShell parser"; then
        ...
        record_failure "R5 — changed PowerShell could not be parsed" \
            "install pwsh and rerun bash tools/checks.sh --section powershell" 127
    fi
fi

if selected python && matches '(^|/).*\.py$|^pyproject\.toml$|^schemas/|^release/evidence/livedev-'; then
    PY_FILES=()
    while IFS= read -r file; do
    ...
    if (( ${#PY_FILES[@]} > 0 )); then
        ...
        if need ruff "Python static analysis"; then
            ...
            record_failure "R5 — Python static analysis could not run" \
                "install ruff and rerun bash tools/checks.sh --section python" 127
        fi
    fi
    if "${PYTHON[@]}" -c 'import pytest' >/dev/null 2>&1; then
        ...
        record_failure "R5/R6 — Python tests could not run" \
            "install pytest and rerun bash tools/checks.sh --section python" 127
    else
...
if selected rust && matches '^(Cargo\.(toml|lock)|crates/|rust-toolchain)'; then
    if need cargo "Rust"; then
        ...
        attempt run "R3/R5 — deterministic optid full-system simulation regressed" \
        ...
        record_failure "R3/R5 — Rust checks could not run" \
            "install cargo and rerun bash tools/checks.sh --section rust" 127
    fi
fi
...
echo "PASS: selected Rush checks passed."
