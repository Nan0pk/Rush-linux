#!/usr/bin/env python3
"""Temporary branch-only script for issue #346; deleted by the committing CI step."""

from pathlib import Path

path = Path("OPTID-COMPLETION-PLAN.md")
...
def replace_section(start: str, end: str, replacement: str) -> None:
    ...
    if text.count(start) != 1 or text.count(end) != 1:
        raise SystemExit(f"section anchors not unique: {start!r} -> {end!r}")
    before, rest = text.split(start, 1)
    _, after = rest.split(end, 1)
    text = before + replacement.rstrip() + "\n\n" + end + after
...
i1 = r'''### I1 — Make diagnostics and configuration friction-free
...
replace_section("### I1 — Make diagnostics and configuration friction-free", "### I2 — Build the full-system simulation and fault matrix", i1)

i2 = r'''### I2 — Build the full-system simulation and fault matrix
...
replace_section("### I2 — Build the full-system simulation and fault matrix", "### I3 — Integrate and promote without turning hardware into a build gate", i2)

i3 = r'''### I3 — Integrate and promote without turning hardware into a build gate
...
replace_section("### I3 — Integrate and promote without turning hardware into a build gate", "## 5. Dependency and parallelization map", i3)
...
required = [
    "sudo optctl doctor --capture",
    "summary.json",
    "latest-control-cycles.jsonl",
    "without random journal grepping",
    "physical-failure reproduction workflow",
    "Hardware promotion fails if a feature works but cannot explain its own result",
    "The 30 packages remain granular",
]
missing = [item for item in required if item not in text]
if missing:
    raise SystemExit(f"missing required amendment text: {missing}")
for heading in (
    ...
    if text.count(heading) != 1:
        raise SystemExit(f"heading count changed: {heading}")
