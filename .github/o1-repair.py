...
source = "crates/optid/src/runtime_observability.rs"
...
"""
replace_once(source, anchor, test + anchor)

replace_once(
    "crates/optid/tests/o1_runtime_status.rs",
...
replace_once(
    ...
    """blocking_reason = "Fresh cold verification at f3cbdb2 found permission-denied PM QoS reported as unsupported, discarded per-device constraint errors, fabricated actual brightness, and hidden directory-discovery failures (docs/inbox/2026-09-06-runtime-observability-verification.md). The field-read defects are repaired with regression coverage; directory-discovery error reporting remains unfinished. Completion still requires a fresh independent passing receipt, so D1 and D2 remain locked. The optctl status surface named in the completion plan is separately unwired by maintainer direction."
""",
    """blocking_reason = "Fresh cold verification at f3cbdb2 found permission-denied PM QoS reported as unsupported, discarded per-device constraint errors, fabricated actual brightness, and hidden directory-discovery failures (docs/inbox/2026-09-06-runtime-observability-verification.md). The field-read defects and source-level directory-discovery reporting are repaired with regression coverage. Completion still requires a fresh independent passing receipt, so D1 and D2 remain locked. The optctl status surface named in the completion plan is separately unwired by maintainer direction."
""",
)
